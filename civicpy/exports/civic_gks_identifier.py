...
# Object types supported by ``compute_civic_gks_identifier``.
CivicGksComputedIdentifierObject: TypeAlias = (
    ClinicalVariantProposition
    | ConditionSet
    | TherapyGroup
...
def compute_civic_gks_identifier(gks_object: BaseModel) -> str:
    ...
    * A nested ``SequenceReference`` contributes its ``refgetAccession`` when it
    ...
    if isinstance(gks_object, ClinicalVariantProposition):
        ...
        identifier_prefix = CivicGksIdentifierPrefix.CONDITION_SET
        member_field = CONDITIONS_FIELD
    elif isinstance(gks_object, TherapyGroup):
        identifier_prefix = CivicGksIdentifierPrefix.THERAPY_GROUP
        member_field = THERAPIES_FIELD
    elif isinstance(gks_object, CivicGksAlleleOriginQualifier):
        identifier_prefix = CivicGksIdentifierPrefix.ALLELE_ORIGIN_QUALIFIER
    ...
    if isinstance(gks_object, CivicGksAlleleOriginQualifier):
    ...
    if member_field and member_field in identity_payload:
        identity_payload[member_field] = sorted(
            identity_payload[member_field], key=serialize_canonical_json
        )
...
def _compute_nested_group_identifier(value: dict[str, Any]) -> str | None:
    ...
    return compute_civic_gks_identifier(group)
